...
The **Cortex Docs PDF Downloader** is an advanced, automated web-scraping and document-generation tool built with Python, Streamlit, and Playwright.

It is specifically designed to interact with the Palo Alto Networks documentation portal (which uses the Fluid-Topics framework). The tool allows users to manage a list of documentation URLs via an Excel file, automatically extract hidden publication dates, and compile massive, multi-page HTML manuals into clean, heavyweight PDF files without floating web widgets obstructing the text.
...
* `Version` (Used to store the extracted date)
* `Status` (Tracks execution: Pending, Downloaded, or Failed)
...
* **💻 Live Execution Logs:** A real-time terminal window inside the app. It mirrors your system's standard terminal output, telling you exactly what the bot is doing at any given second, preventing you from guessing if the app has frozen during massive downloads.
...
This diagram breaks down the exact sequence of events from the moment you open the app to the moment your PDF is safely verified and saved.
...
graph TD
    ...
    Step8B["8. QA File Size Check"]:::appProcess

    %% Final Save
    Step9["9️⃣ Save to Excel & Disk"]:::dataSave
...
* **Step 1: Load Excel Data:** When you launch the Streamlit app, it automatically reads your `sources.xlsx` file. It cleans up the "File Title" column to remove illegal characters and `.pdf` extensions. It ignores blank columns to prevent "nan" visual bugs.
...
* **Step 5 (PDFs):** The bot clicks the on-screen "Print Map" icon, force-checks the "Select All" box, and initiates document compilation. Because files like the XSIAM documentation are often >100 MB, the script deploys a dynamic interceptor that patiently waits (up to 15 minutes) for the backend servers to send a readiness signal in a "Ghost Tab".
* **Step 6 (PDFs):** The **Widget Assassin** activates. It injects CSS into the page to permanently hide floating chatboxes, "Scroll to Top" arrows, and Pendo feedback icons so they don't block the text on your PDF.
* **Step 7 (PDFs):** The clean page is converted into a highly-compressed, A4-sized PDF document.
* **Step 8 (PDFs):** A **Quality Assurance (QA) Check** runs. It physically weighs the downloaded file on your hard drive. If the file is less than 10 KB, the system recognizes it as a failed/blank loading screen and flags an error.

**The Final Step:**

* **Step 9: Save to Excel & Disk:** For dates, the `Version` column in Excel is updated. For PDFs passing the QA check, the file is confirmed on your hard drive, and the `Status` column in Excel is updated to "✅ Downloaded". The UI terminal logs "SUCCESS" and refreshes the table.
...
* **What it does:** Reads the `sources.xlsx` file and dynamically cleans data.
* **Smart URL Extraction:** It looks at the `File Title` column. If the text is a clickable hyperlink, it extracts the hidden URL payload. If it's just a raw text string starting with "http", it extracts that instead.
* **Sanitization:** It looks for `.pdf` in the filename and strips it out, replacing illegal characters (`/`, `\`, `*`, etc.) so the OS won't crash when saving the file. It also ignores headerless Excel columns to prevent `nan` columns from displaying in the UI.
...
* **Shadow DOM Piercing:** The script injects custom JavaScript into the page. This script recursively walks through every HTML node and "Shadow Root" (hidden web components) to pull out all raw text.
...
* **Formatting:** It passes the date to `format_date()`, which uses Pandas to intelligently infer and convert formats like `2026-05-03` into `May 03, 2026` without throwing terminal warnings.

### C. The "Heavyweight PDF Download" Engine
This is the most complex part of the code, designed to bypass enterprise bot-protection and handle massive 100MB+ document compilations without timing out.
...
3.  **The "Print Signal" Interceptor:** Instead of guessing when a massive document is done loading, the script injects a global spy variable (`window.__PRINT_READY__`). It freezes the automation and waits up to **15 minutes** for the web application's native code to explicitly declare that the document is finished compiling.
...
5.  **PDF Generation & QA Verification:** Once the page is clean, it emulates "Print" media and saves the binary file. Finally, it checks the physical file size on the OS level to ensure it is larger than 10 KB, preventing false positives from blank loading screens.
...
* **Empty PDF (Timeout/Blank):** If a document logs `❌ Failed: Empty PDF (Timeout/Blank)`, it means the 15-minute wait limit was exceeded, and the server failed to compile the document in time. The script safely catches this 1 KB file, flags it, and moves to the next guide. Try running the failed guide again later.
* **Handling Crashes ("Context Destroyed"):** If the log shows an error like `Context Destroyed`, the underlying browser likely ran out of memory. The script will safely skip it and move to the next file without crashing the whole application.
* **Background Operation:** The bot runs in `headless=True` mode, meaning the Chrome browser operates entirely invisibly in your computer's RAM. You can continue to use your computer normally while the app downloads heavy PDFs in the background.
